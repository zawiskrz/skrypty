...
echo -e "\n${BLUE}[1/11] Aktualizacja systemu...${NC}"
apt update && apt upgrade -y

# 3. Instalacja pakietów systemowych, X11, LightDM, SPICE i bibliotek build-essential
echo -e "\n${BLUE}[2/11] Instalacja serwera X11, LightDM i narzędzi systemowych...${NC}"
apt install -y \
...
echo -e "\n${BLUE}[3/11] Dodawanie repozytorium i instalacja Visual Studio Code...${NC}"
...
echo -e "\n${BLUE}[4/11] Instalacja przeglądarki LibreWolf i Thunderbird...${NC}"
...
echo -e "\n${BLUE}[5/11] Instalacja LibreOffice (PL + Base) oraz VLC...${NC}"
apt install -y \
  libreoffice \
  libreoffice-l10n-pl \
  libreoffice-help-pl \
...
echo -e "\n${BLUE}[6/11] Instalacja pakietów naukowych Python (Apt)...${NC}"
apt install -y \
...
echo -e "\n${BLUE}[7/11] Instalacja Kitty, jgmenu, Picom, apletów systemowych i motywów GTK...${NC}"
...
# Uruchom jgmenu tylko po kliknięciu prawym przyciskiem w puste tło pulpital
...
XEOF
  ...
  # Utworzenie pliku autostartu z uruchamianiem apletów w tle
  mkdir -p "$REAL_HOME/.config/qtile"
  cat <<'ASEOF' > "$REAL_HOME/.config/qtile/autostart.sh"
#!/bin/bash

# Kompozytor okien
picom &

# Myszka / menu kontekstowe na pulpicie
xbindkeys &
...
xfce4-power-manager &
ASEOF
  chmod +x "$REAL_HOME/.config/qtile/autostart.sh"
EOF

# 9. Instalacja uv oraz Qtile w środowisku użytkownika
echo -e "\n${BLUE}[8/11] Instalacja Astral-UV oraz Qtile dla użytkownika $REAL_USER...${NC}"
sudo -u "$REAL_USER" bash << EOF
...
echo -e "\n${BLUE}[9/11] Rejestracja Qtile w LightDM...${NC}"
...
echo -e "\n${BLUE}[10/11] Kopiowanie plików konfiguracyjnych...${NC}"
